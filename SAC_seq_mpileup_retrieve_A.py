...
for line in sys.stdin:
    ...
    if info[2] in ["A","a"]:
        ...
        mutation_counts = Forward_seq_mutation[3] + Forward_seq_mutation[4] \
                       + Forward_seq_mutation[5] + Forward_seq_mutation[6] + Forward_seq_mutation[7]
        try:
            mutation_ratio = round(mutation_counts / total_counts, 4)
            print(f'{info[0]}\t{info[1]}\t+\t{mutation_ratio}\t{total_counts}\t{mutation_counts}\tmutation_T/G/C={Forward_seq_mutation[5]}:{Forward_seq_mutation[3]}:{Forward_seq_mutation[4]}\t{stop}')
        except:
            print(f'{info[0]}\t{info[1]}\t+\tNA\t{total_counts}\t{mutation_counts}\tmutation_T/G/C={Forward_seq_mutation[5]}:{Forward_seq_mutation[3]}:{Forward_seq_mutation[4]}\t{stop}')